...
func TestFlatLineShouldPass1(t *testing.T) {
	...
	model, err := NewLeastSquares(.01, flatX, flatY)
	...
	assert.InDelta(t, 3, guess[0], 1e-2, "Guess should be really close to 3 (within 1e-2) for a flat line")
...
func TestInclinedLineShouldPass1(t *testing.T) {
	...
	model, err := NewLeastSquares(.01, increasingX, increasingY)
	assert.Nil(t, err, "You should be able to create a new Least Squares model!")
    err = model.Learn()
    assert.Nil(t, err, "Learning error should be nil")
	...
	guess, err = model.Predict([]float64{1})
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 1, guess[0], 1e-2, "Guess should be really close to 1 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")

	guess, err = model.Predict([]float64{0})
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 0, guess[0], 1e-2, "Guess should be really close to 0 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")

	guess, err = model.Predict([]float64{3})
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 3, guess[0], 1e-2, "Guess should be really close to 3 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")

	guess, err = model.Predict([]float64{-2})
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, -2, guess[0], 1e-2, "Guess should be really close to -2 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")
}
...
func TestThreeDimensionalLineShouldPass1(t *testing.T) {
	...
	model, err := NewLeastSquares(.001, threeDLineX, threeDLineY)
	assert.Nil(t, err, "You should be able to create a new Least Squares model!")
    err = model.Learn()
    assert.Nil(t, err, "Learning error should be nil")
	...
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 6, guess[0], 1e-2, "Guess should be really close to 6 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")

	guess, err = model.Predict([]float64{0, 10})
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 10, guess[0], 1e-2, "Guess should be really close to 10 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")
	...
	assert.Len(t, guess, 1, "Length of a LeastSquares model output from the hypothesis should always be a 1 dimensional vector. Never multidimensional.")
	assert.InDelta(t, 4, guess[0], 1e-2, "Guess should be really close to 4 (within 1e-2)")
	assert.Nil(t, err, "Prediction error should be nil")
}
